...
- 最终响应直接从 `## 1. 表清单` 开始，只输出生成的 Markdown 正文，不添加开场白、分析过程、规则说明、校验结果、免责声明或结尾说明。
...
主表和关联表超过一张时，使用 `## 2. 表关系` 标题和 `text` 代码块表达一对一、一对多或多对多关系；除该关系图外，不在最终文档中增加 SQL 代码块或说明代码块。例如：
...
从 `## 3.` 开始，严格按照表清单中的顺序逐张展开具体表。第一张表使用 `## 3. 表名`，下一张表依次使用 `## 4. 表名`；每张表只保留以下两部分：
